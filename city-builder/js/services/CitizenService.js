...
class CitizenService {
  ...
  processTurn() {
    this.#syncEmployeesFromCitizens(); // siempre sincronizar primero
  ...
  // Reconstruye el array employees de cada edificio desde los ciudadanos.
  // Esto corrige la desincronización que ocurre al cargar una partida guardada.
  #syncEmployeesFromCitizens() {
    ...
    // Reasignar desde los ciudadanos que ya tienen jobId
    for (const citizen of state.citizens) {
      if (!citizen.jobId) continue;
      const building = employables.find(b => b.id === citizen.jobId);
      if (building) {
        building.employees.push(citizen.id);
      } else {
        // El edificio ya no existe (fue demolido), liberar al ciudadano
        citizen.jobId = null;
      }
    }
  }
  ...
  #tryGrowPopulation() {
    ...
    const availableCapacity = residentials.reduce(
      (sum, b) => sum + (b.capacity - b.currentOccupants.length), 0
    );
